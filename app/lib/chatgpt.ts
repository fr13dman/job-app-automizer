...
interface GenerateCuratedResumeParams {
    resumeFile: File
    jobDescription: string
    additionalParameters?: string[]
}
...
// Generate a cover letter using the OpenAI API with file input
export const generateCuratedResume = async (params: GenerateCuratedResumeParams) => {
    // Construct the system prompt
    const systemPrompt = `You are the Director of Recruiting at the company in question and an expert resume curator. 
    Curate the resume to match the job description provided. 
    ${params.additionalParameters?.join('\n')}`
    ...
    try {
        ...
        // Enhanced error logging
        logger.error({
            msg: 'Error in generateCuratedResume',
            error:
                error instanceof Error
                    ? {
                          name: error.name,
        ...
        if (error instanceof Error) {
            ...
            throw new OpenAIError(
                `Failed to generate curated resume calling OpenAI: ${error.message}`,
                error
            )
        }
        ...
        throw new OpenAIError(
            'An unexpected error occurred while generating the curated resume calling OpenAI...',
            error
        )
    }
}
